...
export default function PeriodicGame() {
  ...
  const computeTotalSlots = useCallback((): number => {
    ...
    for (let i = 57; i <= 71; i++) {
      if (elements.find(el => el.atomicNumber === i)) count++;
    }
  ...
  const renderPeriodicGrid = () => {
    ...
    for (let period = 1; period <= 7; period++) {
      ...
      for (let group = 1; group <= 18; group++) {
        ...
        const correctElement = getElementByPosition(group, period);
        if (!correctElement) {
          row.push(<div key={key} className="w-12 h-12"></div>);
          continue;
        }
        ...
        row.push(
          <div
            ...
            data-slot={key}
            className={`
              w-12 h-12 border-2 border-gray-600 rounded-md flex items-center justify-center
          ...
          >
            {placedElement ? <div>{placedElement.symbol}</div> : <div className="text-gray-500">&nbsp;</div>}
          </div>
        );
      }
      grid.push(<div key={period} className="flex gap-1 justify-center">{row}</div>);
    }
      ...
      <div className="flex flex-row items-start gap-8">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-col gap-2 mt-4">
            <div className="flex gap-1 justify-center">{lanthanides}</div>
            <div className="flex gap-1 justify-center">{actinides}</div>
          </div>
        </div>
        <div className="w-80">{renderElementsPool()}</div>
      </div>
    );
  };
    ...
    <div className="min-h-screen bg-slate-900 text-white p-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={startGame} className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-md font-semibold">Start</button>
          <button onClick={pauseGame} disabled={!gameState.isPlaying} className="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 disabled:bg-gray-600 rounded-md font-semibold">{gameState.isPaused ? 'Resume' : 'Pause'}</button>
          <button onClick={shuffleElements} disabled={!gameState.isPlaying} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 rounded-md font-semibold">Shuffle</button>
        </div>
      </div>

      <div className="mb-6">{renderLegend()}</div>
      <div className="mb-8">{renderPeriodicGrid()}</div>
      ...
      {(gameState.gameOver || gameState.gameWon) && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-gray-800 p-8 rounded-lg text-center">
            ...
            <button onClick={restartGame} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 rounded-md font-semibold">Play Again</button>
          </div>
        </div>
      )}
      ...
      {!gameState.isPlaying && !gameState.gameOver && !gameState.gameWon && (
        <div className="text-center">
          ...
          <p className="text-gray-300 max-w-2xl mx-auto">
            Drag elements from the side pool to their correct positions in the
